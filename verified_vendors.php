...
.btn { padding: 8px 12px; border: none; border-radius: 6px; cursor: pointer; font-weight: 600; transition: all 0.3s ease; }
.btn-sm { padding: 6px 10px; font-size: 13px; }
.btn-primary { background: linear-gradient(135deg, #1e3a8a 0%, #1e40af 100%); color: white; }
.btn-primary:hover { background: linear-gradient(135deg, #1e40af 0%, #2563eb 100%); transform: translateY(-2px); box-shadow: 0 4px 12px rgba(30, 58, 138, 0.4); }
</style>
</head>
<body>
...
<tr>
<td>
<button class="btn btn-sm btn-primary" onclick="viewDetails(<?php echo $v['id']; ?>, 'individual')" title="View Details">
...
<tr>
<td>
<button class="btn btn-sm btn-primary" onclick="viewDetails(<?php echo $v['id']; ?>, 'business')" title="View Details">
<i class="fas fa-eye"></i>
</button>
</td>
<td><?php echo $sl++; ?></td>
...
// Save scroll position and active tab before page reload
function saveState() {
    var activeTab = $('.tab-button.active').length > 0 ? 
        ($('.tab-button.active').text().includes('Individual') ? 'individual' : 'business') : 'individual';
    
    localStorage.setItem('vendorActiveTab', activeTab);
    
    if (activeTab === 'individual' && individualTable) {
        var scrollPos = $('#individual-tab .dataTables_scrollBody').scrollTop();
        var page = individualTable.page();
        localStorage.setItem('vendorIndividualScroll', scrollPos);
        localStorage.setItem('vendorIndividualPage', page);
    } else if (activeTab === 'business' && businessTable) {
        var scrollPos = $('#business-tab .dataTables_scrollBody').scrollTop();
        var page = businessTable.page();
        localStorage.setItem('vendorBusinessScroll', scrollPos);
        localStorage.setItem('vendorBusinessPage', page);
    }
}

// Restore scroll position and active tab after page load
function restoreState() {
    var activeTab = localStorage.getItem('vendorActiveTab') || 'individual';
    
    // Switch to saved tab first
    ...
    // Restore scroll and page with proper timing
    setTimeout(function() {
        if (activeTab === 'individual' && individualTable) {
            var savedPage = localStorage.getItem('vendorIndividualPage');
            var savedScroll = localStorage.getItem('vendorIndividualScroll');
            
            if (savedPage !== null && savedPage !== 'null') {
                individualTable.page(parseInt(savedPage)).draw(false);
            }
            
            setTimeout(function() {
                if (savedScroll !== null && savedScroll !== 'null') {
                    $('#individual-tab .dataTables_scrollBody').scrollTop(parseInt(savedScroll));
                }
            }, 200);
        } else if (activeTab === 'business' && businessTable) {
            var savedPage = localStorage.getItem('vendorBusinessPage');
            var savedScroll = localStorage.getItem('vendorBusinessScroll');
            
            if (savedPage !== null && savedPage !== 'null') {
                businessTable.page(parseInt(savedPage)).draw(false);
            }
            
            setTimeout(function() {
                if (savedScroll !== null && savedScroll !== 'null') {
                    $('#business-tab .dataTables_scrollBody').scrollTop(parseInt(savedScroll));
                }
            }, 200);
        }
    }, 300);
...
$(document).ready(function() {
    ...
    // Hide loader and show tables after both are initialized
    $('#initial-loader').fadeOut(400, function() {
        $('.table-container').css('opacity', '1');
        // Restore state after both tables are ready
        restoreState();
    });
    
    // Save state on scroll for both tables
    $('#individual-tab .dataTables_scrollBody, #business-tab .dataTables_scrollBody').on('scroll', function() {
        saveState();
    });
    
    // Save state on page change for both tables
    $('#individualTable, #businessTable').on('page.dt', function() {
...
function viewDetails(vendorId, accountType) {
    // Save state before navigation
    saveState();
    // Navigate to vendor details page
    window.location.href = 'vendor-details.php?id=' + vendorId + '&type=' + accountType;
}
</script>
</body>
</html>
