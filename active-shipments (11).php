...
function wa_send_template($phone, $template_name, $variables) {
    ...
    $templates_with_button = [
        'shipment_out_for_delivery_cod',
        'shipment_delivered_cod_v2',
        'shipment_delivered_prepaid',
    ];
        ...
        if (!empty($tn) && !empty($ns)) {
            ...
            if ($stmt) {
                ...
                if ($rl = mysqli_fetch_assoc($res)) {
                    ...
                    // Define status sequence with detailed status messages
                    $status_sequence = [
                        ...
                        'Picked up' => 'Package has been picked up from sender', // Handle lowercase variant
                    ...
                    $mandatory_statuses = ['Pickup Assigned', 'Picked Up', 'Picked up'];
                    ...
                    // CRITICAL CHECK: Only block if trying to jump to post-pickup statuses without completing pickup
                    // Check CURRENT status in courier table, not history
                    if ($new_pos !== false && $new_pos > 2) {
                        // Trying to jump to Received at Office (3) or later
                        // Check if current status is already at or past "Picked Up" (position 2)
                        if ($current_pos < 2) {
                            // Current status is before "Picked Up" - block the update
                            error_log("Quick Update: BLOCKED - Current status ($current_status at pos $current_pos) is before Picked Up");
                            $_SESSION['update_message'] = array('type' => 'danger', 'text' => '⚠️ Cannot skip pickup steps! Please complete "Pickup Assigned" and "Picked Up" first.');
                            header("Location: " . basename(__FILE__));
                            exit();
                        } else {
                            error_log("Quick Update: Allowed - Current status ($current_status at pos $current_pos) is at or past Picked Up");
                        }
                    } else {
                        error_log("Quick Update: No pickup validation needed (new_pos=$new_pos is <= 2)");
                    }
                    ...
                    if ($su) {
                        ...
                        if (mysqli_stmt_execute($su)) {
                            ...
                            error_log("WA NOTIFY: tracking=$wa_tracking status=$wa_status paymode=$wa_paymode sender=$wa_sender_fmt is_cod=" . ($is_cod ? 'yes' : 'no'));
                            ...
                            if (stripos($wa_status, 'delivered') !== false && stripos($wa_status, 'out for') === false) {
                                ...
                                if ($wa_sender_fmt) {
                                    wa_send_template($wa_sender_fmt, $tpl, $vars);
                                }
                                if ($wa_receiver_fmt && $wa_receiver_fmt !== $wa_sender_fmt) {
                                    sleep(1);
                                    wa_send_template($wa_receiver_fmt, $tpl, $vars);
                                }
                            }
                            ...
                            if ($new_pos !== false && $new_pos > $current_pos) {
                                ...
                                for ($i = $current_pos + 1; $i <= $new_pos; $i++) {
                                    ...
                                    // Add small time offset for each status (1 minute apart)
                                    $status_time = date('Y-m-d H:i:s', strtotime($now) + ($i - $current_pos - 1) * 60);
